// AES round functions for CryptoNight (WebGPU/WGSL)
// Translated from src/backend/opencl/cl/cn/wolf-aes.cl

fn byte_val(x: u32, y: u32) -> u32 {
    return (x >> (y * 8u)) & 0xFFu;
}

// AES0_C is the only constant table; AES1/AES2/AES3 are derived by rotation.
const AES0_C: array<u32, 256> = array<u32, 256>(
    ...
    0x60202040u, 0x1FFCFCF3u, 0xC8B1B179u, 0xED5B5BB6u,
...
fn sub_word(inw: u32) -> u32 {
    return (SBOX[byte_val(inw, 3)] << 24u) |
           (SBOX[byte_val(inw, 2)] << 16u) |
           (SBOX[byte_val(inw, 1)] <<  8u) |
            SBOX[byte_val(inw, 0)];
}

// One AES round using four lookup tables (full AES_Round from wolf-aes.cl)
fn aes_round(
...
// Two-table AES round (AES_Round_Two_Tables from wolf-aes.cl)
...
// AES-256 key expansion (AESExpandKey256 from wolf-aes.cl)
