...
if (CMAKE_CXX_COMPILER_ID MATCHES GNU)
    ...
    if (EMSCRIPTEN)
        set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -s WASM=1 -s ALLOW_MEMORY_GROWTH=1 -s INITIAL_MEMORY=536870912 -s MAXIMUM_MEMORY=4294967296 -s EXPORTED_RUNTIME_METHODS=['ccall','cwrap','callMain','FS'] -s EXPORTED_FUNCTIONS=['_main','_malloc','_free'] -s USE_PTHREADS=1 -s PTHREAD_POOL_SIZE=16 -s PTHREAD_POOL_SIZE_STRICT=0 -s MODULARIZE=1 -s EXPORT_NAME=\"createXMRig\"")
        set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -pthread -msimd128 -msse -msse2 -fopenmp -I${CMAKE_SOURCE_DIR}/libomp")
        set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -pthread -msimd128 -msse -msse2 -fopenmp -I${CMAKE_SOURCE_DIR}/libomp")
        set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -fopenmp -L${CMAKE_SOURCE_DIR}/libomp -lomp")
    elseif (WIN32)
...
elseif (CMAKE_CXX_COMPILER_ID MATCHES Clang)
    ...
    if (EMSCRIPTEN)
        set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -s WASM=1 -s ALLOW_MEMORY_GROWTH=1 -s INITIAL_MEMORY=536870912 -s MAXIMUM_MEMORY=4294967296 -s EXPORTED_RUNTIME_METHODS=['ccall','cwrap','callMain','FS'] -s EXPORTED_FUNCTIONS=['_main','_malloc','_free'] -s USE_PTHREADS=1 -s PTHREAD_POOL_SIZE=4 -s PTHREAD_POOL_SIZE_STRICT=0 -s MODULARIZE=1 -s EXPORT_NAME=\"createXMRig\"")
        set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -pthread -msimd128 -msse -msse2 -fopenmp -I${CMAKE_SOURCE_DIR}/libomp")
        set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -pthread -msimd128 -msse -msse2 -fopenmp -I${CMAKE_SOURCE_DIR}/libomp")
        set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -fopenmp -L${CMAKE_SOURCE_DIR}/libomp -lomp")
    elseif ((WIN32 AND ARM_TARGET) OR BUILD_STATIC)
        set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -static")
    endif()
endif()
